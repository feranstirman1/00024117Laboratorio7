...
Matrix createLocalK(int e,mesh &m){
    float t,k,delta,lambda,Ae,J,D;
    
    //Componentes de K
    // [ A+K  G-H ]
    // [  D    0  ]
    Matrix matrixA,matrixK,matrixG,matrixH,matrixHH,matrixD;
    ...
    productRealMatrix(2*lambda*J/(3*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);

    //Preparando matrixH (En clase conocida simplemente como H)
    delta = m.getParameter(DELTA);
    productRealMatrix(delta/(8*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixH);
    ...
    productRealMatrix(-1, matrixH, matrixHH);
    //Colocando submatrices en K
    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixA,matrixK,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixG,matrixHH,6,3));
